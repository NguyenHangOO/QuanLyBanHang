...
namespace GUI_QLBH
{
    public class Edit
    {
        public static void chinhanh(PictureEdit hinh)
        {
            System.Drawing.Drawing2D.GraphicsPath gp2 = new System.Drawing.Drawing2D.GraphicsPath();
            gp2.AddEllipse(0, 0, hinh.Width -2, hinh.Height -2);
            Region rg2 = new Region(gp2);
            hinh.Region = rg2;
        }
        public static bool IsNumber(string pvalue)///kiểm tra số trong một chuỗi
        {
            ...
            for (int i = 0; i <= mLen; i++)
            {
                ...
                if (mLen == i) // Chữ số cuối cùng không cần xét tiếp break; 
                    switch ((mLen - i) % 9)
                    {
                        case 9:
                            mTemp = mTemp + " tỷ";
                            if (sNumber.Substring(i + 1, 3) == "000000000") i = i + 3;
                            if (sNumber.Substring(i + 1, 3) == "000000000") i = i + 3;
                            if (sNumber.Substring(i + 1, 3) == "000000000") i = i + 3;
                            break;
                        case 6:
                            mTemp = mTemp + " triệu";
                            if (sNumber.Substring(i + 1, 3) == "000000") i = i + 3;
                            if (sNumber.Substring(i + 1, 3) == "000000") i = i + 3;
                            break;
                        case 3:
                            mTemp = mTemp + " nghìn";
